...
public class AcroGame extends ZugArea {
    enum AcroOption { acroTime, voteTime, acroBaseDisplayTime, acroDisplayTime, nextRoundTime, topicTime, summaryTime,
        victoryPoints, minLetters, maxLetters, voteBonus, speedBonus, adult, maxGameIdle, maxPlayerIdle}
    ...
    public AcroGame(String t, ZugUser c, AreaListener l, String letterFile, int winPoints, int acroTime, int voteTime, boolean allowGuests, boolean adult) {
        ...
        setOptionsManager(new OptionsManager(
                ...
                OptionsManager.createOption(AcroOption.nextRoundTime,5,5,60,1,"Topic Time","Topic Selection Time (in seconds)"),
    ...
    private void nextRound() {
        getPlayers().forEach(player -> player.currentAcro = null);
        ...
        pm().newPhase(AcroPhase.composing, om().getInt(AcroOption.acroTime) * 1000, ZugUtils.newJSON().put(AcroField.acro, getCurrentAcro()))
                .thenRun(() -> {
                    initVoting();
                    spam("Round " + round + ": enter your votes!");
                    pm().newPhase(AcroPhase.voting,om().getInt(AcroOption.voteTime) * 1000,acrosToJSON(true))
                            .thenRun(() -> {
                                tally();
    ...
    public void initVoting() {
        ...
        Collections.shuffle(acros);
        int i = 0; for (Acro acro : acros) acro.id = String.valueOf(++i);
    }
    ...
    List<Acro> getCurrentAcros() {
        return getPlayers().stream().filter(p -> p.currentAcro != null).map(p2 -> p2.currentAcro).toList();
    }

    List<AcroPlayer> getPlayers() {
        return getOccupants().stream().map(x -> (AcroPlayer) x).toList();
    }
